...
export function schemaToObject(schema: any): any {
  switch (schema.type) {
    case 'object': {
      if (schema.keys) {
    ...
    default: {
      const presence =
        schema.flags && schema.flags.presence
          ...
          : '';
      if (schema.flags && schema.flags.only) {
        if (schema.allow.length === 1) {
          return `'${schema.allow[0]}'`;
        }
        let allowed = schema.allow.map((v: string) => `'${v}'`).join('|');
...
export default function createApiDocumentation(options: ApiDocOptions): string {
  const data = options.groups.map((group) => {
    const routes = group.routes.map((route) => {
      ...
      const useManualOutputDocs = Boolean(route.documentation.outputs);
      const output = route.documentation.outputs ?? input.output ?? {};
      return {
        ...route,
        documentation: {
